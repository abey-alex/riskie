...
class TransactionService {
    ...
    getList = (
        ...
        // Find the parent
        const parentObject: Transaction = this.findTransactionById(
            transactionId,
        )
        if (!parentObject) return []

        // Find all children of this parent
        const children: Array<Transaction> = this.findChildrenWithConfidenceLevel(
            parentObject,
            confidenceLevel,
        ).map(
        ...
        if (node.children?.length) {
            ...
            while (children.length) {
                ...
                child.combinedConnectionInfo = {
                    confidence:
                        child.connectionInfo?.confidence || 1 * confidenceLevel,
                    types: [
                ...
                childTransactions.push(
                    ...this.findChildren(
                        child,
                        child.combinedConnectionInfo.confidence,
                    ),
                )
            }
        }
     ...
     */
    findChildrenWithConfidenceLevel = (
        node: Transaction,
        confidenceLevel: number,
    ): Array<Transaction> => {
        ...
        return childTransactions.filter(
            (child: Transaction): boolean =>
                child.combinedConnectionInfo.confidence >= confidenceLevel,
        )
    }
}
